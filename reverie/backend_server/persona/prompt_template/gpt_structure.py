...
Modified: Added Ollama support

File: gpt_structure.py
Description: Wrapper functions for calling OpenAI APIs or Ollama APIs.
"""
...
# Import OpenAI only if using OpenAI provider
if llm_provider == "openai":
    import openai

    openai.api_key = openai_api_key

# Import Ollama interface if using Ollama provider
...
def ChatGPT_single_request(prompt):
    ...
    if llm_provider == "ollama":
        return ollama_chat_request(prompt, model=ollama_model_name)
    else:
...
def GPT4_request(prompt):
    ...
    server or Ollama and returns the response.
    ARGS:
      prompt: a str prompt
    ...
    try:
        if llm_provider == "ollama":
            # Use Ollama with the configured model (can be a more powerful model)
            return ollama_chat_request(prompt, model=ollama_model_name)
        else:
            completion = openai.ChatCompletion.create(
...
def GPT_request(prompt, gpt_parameter):
    ...
    try:
        ...
        print(f"TOKEN LIMIT EXCEEDED or ERROR: {str(e)}")
...
def get_embedding(text, model=None):
    ...
    ARGS:
      text: text to embed
      model: model name (optional, uses default from utils)
    RETURNS:
    ...
    if llm_provider == "ollama":
        ...
        return ollama_get_embedding(text, model=model)
    else:
        if model is None:
